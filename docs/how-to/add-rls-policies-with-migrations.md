...
- **Running on a non-PostgreSQL database.** Not a pitfall: `EnableRLS`, `CreateTenantPolicy` and `DropTenantPolicy` apply on PostgreSQL and are a logged no-op anywhere else, emitting one `logger.info` line each on the `boundary.migrations` logger (naming the operation, the model, the alias and the vendor) and no DDL. Keep one set of migration files and run them unchanged on SQLite locally; do not wrap them in a backend conditional and do not route them away. What you lose on that backend is the RLS layer itself, not the migration: the model keeps its ORM-layer filtering, and `boundary.E006`/`boundary.W003` go quiet, so a clean `check` there says nothing about the database layer. `AdoptTenantApp` is the exception and still refuses off PostgreSQL, because an adopted table has no ORM layer beneath the policy.
