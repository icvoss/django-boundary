...
| `boundary.W002` | Warning | Two or more known tenant-resolution middlewares are in `MIDDLEWARE`, which double-resolves the tenant. The canonical external middleware is `icv_tenants.middleware.TenantContextMiddleware`; the legacy identity path remains recognised during migration |
